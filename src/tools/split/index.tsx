...
export default function SplitTool() {
  ...
  const addRange = () => setRanges((prev) => [...prev, { id: Date.now(), from: "", to: "" }]);
  const removeRange = (id: number) => setRanges((prev) => prev.length > 1 ? prev.filter((r) => r.id !== id) : prev);
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (outputMode === "single" || parsed.length === 1) {
        ...
        const filename = file.name.replace(/\.pdf$/i, "-split.pdf");
        setResult({ data, filename });
      } else {
        // zip — download directly, no chaining
        const zip = new JSZip();
        for (let i = 0; i < parsed.length; i++) {
          setProgress(`Building range ${i + 1}/${parsed.length}…`);
          const out = await PDFDocument.create();
  ...
  return (
    <div style={{ maxWidth: 580, margin: "0 auto", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.875rem" }}>Define one or more page ranges to extract.</p>
      </div>
      ...
      {!file && !reading ? (
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: "0.65rem 1rem", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.875rem" }}>
          <span>
            ...
            <span style={{ color: "var(--text-muted)", marginLeft: "0.5rem" }}>— {pageCount} page{pageCount !== 1 ? "s" : ""}</span>
          </span>
          <button onClick={() => { setFile(null); setPageCount(null); setRanges([{ id: 1, from: "", to: "" }]); setError(null); setResult(null); }} style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", padding: "0 0.25rem", fontSize: "1rem" }}>✕</button>
        </div>
      )}
      ...
      {file && pageCount !== null && !result && (
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: "0.25rem" }}>Page ranges</div>

            {ranges.map((r, idx) => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: "0.65rem 0.875rem" }}>
                <span style={{ fontSize: "0.8rem", color: "var(--text-muted)", minWidth: 56 }}>Range {idx + 1}</span>
                <input type="number" min={1} max={pageCount} value={r.from} onChange={(e) => updateRange(r.id, "from", e.target.value)} placeholder="From" style={inputStyle} />
                <span style={{ color: "var(--text-muted)", fontSize: "0.8rem" }}>–</span>
                <input type="number" min={1} max={pageCount} value={r.to} onChange={(e) => updateRange(r.id, "to", e.target.value)} placeholder="To" style={inputStyle} />
                {r.from && r.to && parseRange(r.from, r.to, pageCount) && (
                  <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>{parseRange(r.from, r.to, pageCount)!.length}p</span>
                )}
                <button onClick={() => removeRange(r.id)} disabled={ranges.length === 1} style={{ marginLeft: "auto", background: "none", border: "none", color: ranges.length === 1 ? "#333" : "var(--text-muted)", cursor: ranges.length === 1 ? "default" : "pointer", display: "flex", alignItems: "center", padding: 2 }}>
                  <X size={15} />
                </button>
              </div>
            ))}
            ...
            <button
              onClick={addRange}
              style={{ display: "flex", alignItems: "center", gap: "0.4rem", background: "none", border: "1px dashed var(--border)", color: "var(--text-muted)", borderRadius: 8, padding: "0.55rem 0.875rem", fontSize: "0.8rem", cursor: "pointer", transition: "border-color 0.15s, color 0.15s" }}
          ...
          {ranges.length > 1 && (
            <div style={{ display: "flex", gap: "0.5rem" }}>
              {(["separate", "single"] as OutputMode[]).map((m) => (
                <button key={m} onClick={() => setOutputMode(m)} style={{ flex: 1, padding: "0.5rem", background: outputMode === m ? "var(--accent)" : "var(--bg-card)", border: `1px solid ${outputMode === m ? "var(--accent)" : "var(--border)"}`, color: outputMode === m ? "#fff" : "var(--text-muted)", borderRadius: 8, fontWeight: outputMode === m ? 600 : 400, cursor: "pointer", fontSize: "0.8rem", transition: "all 0.15s" }}>
      ...
      {result && (
        <ResultActions
          ...
          nextTools={[{ path: "/compress", label: "Compress" }]}
        />
      )}
    </div>
  );
